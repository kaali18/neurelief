...
class _SensoryPatternGameState extends State<SensoryPatternGame> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _controller = AnimationController(
      vsync: this,
      duration: animationDuration, // Set default duration here
    );
    _generateNewPattern();
  }
  ...
  void _showPattern() {
    _controller.reset();
    _controller.duration = animationDuration; // Ensure duration is set before forward
    for (int index in targetPattern) {
      _controller.forward(from: 0).then((_) {
        if (mounted) {
          setState(() {
      ...
      // Reset to 0 for the next iteration to avoid overlapping animations
